...
function usePollingData(userId, intervalMs = 1500) {
  ...
  const [heroSource, setHeroSource] = useState(null) // 'note' | 'screen'
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled) {
          // Detect which changed most recently on the client
          ...
          if (noteChanged && !screenChanged) {
            ...
            // If both changed, prefer screen as the hero
            setHeroSource('screen')
          } else if (!heroSource) {
            // Initial hero if none set
...
export default function PeekScreenPage() {
  ...
  // Decide layout based on data and heroSource
  ...
  const showCenterPreview = revealing && (hasScreenshot || hasUrl || hasNote || hasContact)
  ...
  return (
    ...
    >
      {/* Black base; no UI when not revealing */}
      {revealing && (
        <div className="absolute inset-0">
          {/* Corner note (when not hero) */}
